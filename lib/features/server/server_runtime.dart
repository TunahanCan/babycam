...
class ServerRuntime {
  ...
  Future<void> markClientPaired() async => _emit(_stateForPhase(ServerRuntimePhase.clientPaired));

  Future<void> onClientPaired(Object client) => markClientPaired();

  Future<void> startStreamSession(String clientId, StreamSessionOptions options) async {
    _activeSessions[clientId] = options;
    await _recomputeResources(startMediaIfNeeded: true, phase: ServerRuntimePhase.mediaActive);
  }

  Future<void> stopStreamSession(String clientId) => endSession(clientId);

  Future<void> startMediaRuntimeForSession(String sessionId) => startStreamSession(sessionId, const StreamSessionOptions());
  ...
  Future<void> enableNotificationsForClient(String clientId, {required bool cry, required bool motion}) async {
    _notificationClients[clientId] = (cry: cry, motion: motion);
    await _recomputeResources(startMediaIfNeeded: cry || motion, phase: ServerRuntimePhase.mediaActive);
  }
  ...
  Future<void> stopMediaRuntimeIfNoActiveClients() async {
    await _recomputeResources(startMediaIfNeeded: false, phase: ServerRuntimePhase.mediaIdle);
    if (!_resources.needsVideoCapture && !_resources.needsAudioCapture) {
      await _mediaRuntime.stop();
    }
    _emit(_stateForPhase(_resources.hasNotificationDemand ? ServerRuntimePhase.mediaIdle : ServerRuntimePhase.mediaIdle));
  }
  ...
  Future<void> _recomputeResources({required bool startMediaIfNeeded, required ServerRuntimePhase phase}) async {
    _resources.activeVideoClients = _activeSessions.values.where((s) => s.video).length;
    _resources.activeAudioClients = _activeSessions.values.where((s) => s.audio).length;
    _resources.activeEventClients = _notificationClients.length;
    _resources.wantsCryDetection = _notificationClients.values.any((s) => s.cry);
    _resources.wantsMotionDetection = _notificationClients.values.any((s) => s.motion);
    if (startMediaIfNeeded && (_resources.needsVideoCapture || _resources.needsAudioCapture)) {
