...
enum MiuCamRouteAuthMode { none, bearer, streamToken }

typedef MiuCamRouteHandler = Future<void> Function(
  HttpRequest request,
  String? clientId,
);

class MiuCamHttpRoute {
  const MiuCamHttpRoute(
    this.path,
    this.authMode,
    this.allowedMethods,
    this.handle,
  ...
  final String path;
  final MiuCamRouteAuthMode authMode;
  final Set<String> allowedMethods;
  final MiuCamRouteHandler handle;

  bool allowsMethod(String method) => allowedMethods.contains(method);
}

typedef MiuCamRouteAuthorization = ({bool ok, String? clientId});
...
class MiuCamHttpDispatcher {
  MiuCamHttpDispatcher({
    required Iterable<MiuCamHttpRoute> routes,
    ...
    required this.onLog,
  }) : _routes = Map<String, MiuCamHttpRoute>.unmodifiable({
          for (final route in routes) route.path: route,
        }) {
    if (_routes.length != routes.length) {
      throw StateError('Duplicate MiuCam HTTP route path.');
    }
  }

  final Map<String, MiuCamHttpRoute> _routes;
  ...
  final Future<MiuCamRouteAuthorization> Function(
    HttpRequest request,
