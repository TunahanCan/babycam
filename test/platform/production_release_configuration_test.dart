...
void main() {
  ...
  test('mobile platforms share the production application identity', () {
    ...
    final kotlinFiles = Directory('android/app/src/main/kotlin/com/mimicam/app')
        .listSync()
        .whereType<File>()
        .where((file) => file.path.endsWith('.kt'))
        .toList();

    expect(gradle, contains('namespace = "com.mimicam.app"'));
    expect(gradle, contains('applicationId = "com.mimicam.app"'));
    expect(xcode, contains('PRODUCT_BUNDLE_IDENTIFIER = com.mimicam.app;'));
    expect(kotlinFiles, hasLength(6));
    for (final file in kotlinFiles) {
      expect(file.readAsStringSync(), startsWith('package com.mimicam.app'));
    }
  });
}
